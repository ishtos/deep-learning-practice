...
def build_combined(generator, discriminator):
    combined_input = keras.Input(shape=(latent_dim,))
    combined_label_input = keras.Input(shape=(CLASS_NUM,))
    combined_concat_input = layers.Concatenate(axis=-1)([combined_input, combined_label_input])
    combined_generator_output = generator(combined_concat_input)
    combined_image_input = keras.Input(shape=(height, width, CLASS_NUM))
    combined_concat_image_input = layers.Concatenate(axis=3)([combined_generator_output, combined_image_input])

    discriminator.trainable = False
    combined_discriminator_output = discriminator(combined_concat_image_input)
    combined = keras.models.Model([combined_input, combined_label_input, combined_image_input], combined_discriminator_output)
...
def main(args):
    ...
    x_train = x_train.reshape(x_train.shape[0], height, width, channels).astype(np.float32) # / 255.
    ...
    combined_optimizer = keras.optimizers.Adam(lr=0.1, decay=1.00004)
    ...
    for step in tqdm(range(iterations)):
        ...
        random_labels = np.random.randint(0, CLASS_NUM, batch_size)
        random_labels_vectors = keras.utils.to_categorical(random_labels, CLASS_NUM)
        a_loss = combined.train_on_batch([random_latent_vectors, random_labels_vectors, random_labels_images], misleading_targets)
        ...
        if step % 1000 == 0 and step != 0:
            ...
            img = image.array_to_img(generated_iamges[0] * 255., scale=False) # change
            img.save(os.path.join(save_dir, 'generated' + str(step) + '.png'))

            img = image.array_to_img(real_images[0] * 255., scale=False) # change
